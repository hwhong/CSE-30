 ...
 * Date: 4/2/17 
 * Sources of Help: Tutor, websites 
 */ 
...
void testdisplayLights() {
  ...
  lightBank[1] = 0xABCDEF12;
  expected = "*-** *-*- ***- ---* --** *-** *-*- ***- "
             "*-*- *-** **-- **-* ***- **** ---* --*- \n";
  runTest( lightBank, expected, "Test: 0xBAE13BAE, 0xABCDEF1" );

  /* Test 4 */
  lightBank[0] = 0xCDCDCDCD;
  lightBank[1] = 0x19961129;
  expected = "**-- **-* **-- **-* **-- **-* **-- **-* "
             "---* *--* *--* -**- ---* ---* --*- *--* \n";
  runTest( lightBank, expected, "Test: 0xCDCDCDCD, 0x19961129" );

  /* Test 5 */
  lightBank[0] = 0xA774F082;
  lightBank[1] = 0x22191515;
  expected = "*-*- -*** -*** -*-- **** ---- *--- --*- "
             "--*- --*- ---* *--* ---* -*-* ---* -*-* \n";
  runTest( lightBank, expected, "Test: 0xA774F082, 0x22191515" );

  /* Test 6 */
  lightBank[0] = 0x77777777;
  lightBank[1] = 0x01010101;
  expected = "-*** -*** -*** -*** -*** -*** -*** -*** "
             "---- ---* ---- ---* ---- ---* ---- ---* \n";
  runTest( lightBank, expected, "Test: 0x77777777, 0x01010101" );
